...
export default function Navbar (props) {
    ...
    return (
        ...
        <a href="https://flowbite.com/" class="flex items-center">
            <img src="https://scratch.mit.edu/images/scratch-og.png" class="rounded mr-3 h-12 sm:h-9" alt="Scratch Logo" />
            <span class="self-center text-xl font-semibold whitespace-nowrap dark:text-white">Scratch Coding</span>
        </a>
        ...
        <div class="hidden justify-between items-center w-full md:flex md:w-auto md:order-1" id="mobile-menu-4">
            <ul class="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
            {
                links.map((link) =>{
                    let classStyles = "";
                    if(props.current===link){
                        classStyles = linkStyles[0];
                    } else {
                        classStyles = linkStyles[1];
                    }
                    return (
                    <li>
                        <a href="#" className={classStyles}>{link}</a>
                    </li>
                    )
                })
            }
